...
# all ChatGPT experimental runs
runs = [f for f in listdir("../GPT_completions/") if f.endswith(".txt")]

# no run is excluded here
to_exclude = []
runs = [r for r in runs if r not in to_exclude]
...
for r in runs:
	...
	with open("../GPT_completions/" + r) as r:
		r = [answer + "." for answer in r.read().split(".")[:-1] if answer] # GPT stoppend producing an answer at fullstop. We also have an extra newline at the end
