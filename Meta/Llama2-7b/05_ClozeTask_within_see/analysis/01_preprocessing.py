...
for r in runs:
	...
	with open("../completions/" + r) as r:
		r = [answer.strip() for answer in r.read().split("Item: ") if answer] # GPT stoppend producing an answer at fullstop. We also have an extra newline at the end
		...
		for line in r:
			...
			if split_line[2] == "no":
				...
				# answer
				answer.append(re.findall(", but (.*).", line)[0].split()[2:])
			else:
				polarity.append("affirmative")
				# answer
				answer.append(re.findall("and (.*).", line)[0].split()[2:])

			# item
			stop_words = set(stopwords.words('english')) 
			item.append([w for w in split_line if not w in stop_words][2].strip(","))
